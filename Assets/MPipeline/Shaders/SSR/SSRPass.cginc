...
float4 ScreenSpaceReflection_HiZTrace(PixelInput i) : SV_Target
{
	...
	float3 ViewNormal = normalize(mul((float3x3)(worldToCamera), WorldNormal));

	float3 ScreenPos = float3(UV * 2 - 1, SceneDepth);
	float4 ViewPos = mul(inverseProj, float4(ScreenPos, 1));
	ViewPos /= ViewPos.w;*/
	float2 UV = i.uv.xy;
	...
	float3 ViewNormal = mul((float3x3)(worldToCamera), WorldNormal);

	float3 ScreenPos = GetScreenPos(UV, SceneDepth);
	float3 WorldPos = GetWorlPos(ScreenPos, _InvVP);
	float3 ViewPos = GetViewPos(ScreenPos, inverseProj);
	float3 ViewDir = GetViewDir(WorldPos, ViewPos);

	float2 E = MNoise(UV);
	E.y = lerp(E.y, 0.0, brdfBias);
	...
	float4 rayProj = mul ( projection, float4(ViewPos.xyz + ReflectionDir, 1.0) );
	float3 rayDir = normalize( (rayProj.xyz / rayProj.w) - ScreenPos);
	rayDir.xy *= 0.5;

	float4 RayHitData = Hierarchical_Z_Trace(HiZ_MaxLevel, HiZ_StartLevel, HiZ_StopLevel, HiZ_RaySteps, Thickness, 1 / rayCastSize, rayStart, rayDir, _SSR_HierarchicalDepth_RT, sampler_SSR_HierarchicalDepth_RT);
	RayHitData.a = Square( RayHitData.a * GetScreenFadeBord(RayHitData.xy, ScreenFade) );
...
float4 ScreenSpaceReflection_GetColor(PixelInput i) : SV_Target
{
	...
	for (int i = 0; i < 9; i++)
	{
		Offset_UV = mul(OffsetRotationMatrix, offset[i] * (1 / ScreenSize.xy));
		Neighbor_UV = UV + Offset_UV;

		float4 HitUV_PDF = tex2D(_SSR_RayCastRT, Neighbor_UV);
		float3 Hit_ViewPos = GetViewPos(GetScreenPos(HitUV_PDF.rg, HitUV_PDF.b), inverseProj);
...
float4 ScreenSpaceReflection_Temporalfilter(PixelInput i) : SV_Target
{
	...
	float smoothness = tex2D(_DownSampledGBuffer1, UV).a;
	/////Get Reprojection Velocity
	float2 Depth_Velocity = tex2D(_CameraMotionVectorsTexture, UV);
	float2 Ray_Velocity = GetRayMotionVector(HitDepth, UV, _InvVP, _LastVp, viewProjection);
	...
	//ResolverAABB(_SSR_GetSSRColor_RT, 0, 10, TemporalScale, UV, ScreenSize, SSR_MinColor, SSR_MaxColor, SSR_CurrColor);
	...
	float4 lastWorldPos = mul(inverseLastVP, float4(prevProjCoord, lastDepth, 1));
