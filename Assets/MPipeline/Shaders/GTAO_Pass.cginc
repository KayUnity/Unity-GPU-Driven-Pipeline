...
void ResolveGTAO_frag(PixelInput IN, out float2 AO : SV_Target0, out float3 BentNormal : SV_Target1)
{
	...
	float Depth = 0;
	float4 GT_Details = GTAO(uv, (int)dirSampler, (int)sliceSampler, Depth);

	AO = float2(GT_Details.a, Depth);
	BentNormal = mul((float3x3)cameraToWorld, float3(GT_Details.rg, -GT_Details.b));
} 
...
float2 SpatialGTAO_Y_frag(PixelInput IN) : SV_Target
{
	...
	float4 worldPos = mul(inverseVP, float4(float3(uv * 2 - 1, Depth), 1));
	...
	return lerp(1, float2(AO.r, GTRO), intensity);
} 
float2 _Jitter;
float2 _LastJitter;
//////Temporal filter
float4 TemporalGTAO_frag(PixelInput IN) : SV_Target
{
	...
	ResolverAABB(_UpSampleRT, 0, 0, temporalScale, uv, texelSize.zw, minColor, maxColor, filterColor);
...
{
	float2 uv = IN.uv.xy;
	return Upsampling(uv, texelSize.zw, _CameraDepthTexture, _GTAO_Spatial_Texture, _CameraGBufferTexture2);
}