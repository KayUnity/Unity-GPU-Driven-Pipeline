...
inline float CrossBilateralWeight(float r, float d, float d0) {
    ...
    float dz = (d0 - d) * _ProjectionParams.z * sharpness;
...
inline float3 GetPosition(float2 uv)
{
	...
	return float3((uv * uvToView.xy + uvToView.zw) * viewDepth, viewDepth);
}

inline float3 GetNormal(float2 uv)
{
	float3 Normal = tex2D(_DownSampledGBuffer2, uv).rgb * 2 - 1; 
	float3 view_Normal = normalize(mul((float3x3) worldToCamera, Normal));
...
{
	int2 position = (int2)(uv * texelSize.zw);
...
float4 GTAO(float2 uv, int NumCircle, int NumSlice, inout float Depth)
{
	...
	float stepRadius = max(min((radius * halfProjScale) / vPos.b, 512), (float)NumSlice);
	...
	{
		angle = (i + noiseDirection + temporalDirection) * (UNITY_PI / (float)NumCircle);
		sliceDir = float3(float2(cos(angle), sin(angle)), 0);
		slideDir_TexelSize = sliceDir.xy * texelSize.xy;
		h = -1;
	...
	Occlusion = saturate(pow(Occlusion / (float)NumCircle, power));
	Depth = vPos.b;
