    ...
    public unsafe struct TerrainQuadTree
    {
        ...
        public void PushDrawRequest(NativeList<MTerrain.TerrainChunkBuffer> loadedBufferList)
        {
            ...
            if(isRendering)
            {
                ...
                {
                    minMaxHeight = 0,
                    scale = (float)(setting->largestChunkSize / pow(2, lodLevel)),
        ...
        public void CheckUpdate(double2 camXZPos)
        {
            ...
            double dist = distancesq(worldPos, camXZPos);
            if (dist > setting->allLodLevles[lodLevel])
            {
                Combine(lodLevel != 0);
            }
            else if (dist > setting->allLodLevles[lodLevel + 1])
            {
                Combine(true);
            }
            else
